...
/// Load nth user app at -> 2024s-rcore-Richard5038/user/build.py
/// [APP_BASE_ADDRESS + n * APP_SIZE_LIMIT（一般为0x20000）, APP_BASE_ADDRESS（起始地址，一般为0x80400000） + (n+1) * APP_SIZE_LIMIT).
pub fn load_apps() {
    //外部c函数声明
    extern "C" {
        fn _num_app();
    ...
    let num_app = get_num_app();//获得程序数量
    //从num_app_ptr开始，创建一个切片，包含所有应用的起始地址add(1)
